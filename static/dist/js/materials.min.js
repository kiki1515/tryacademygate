class MaterialsHub{constructor(app){this.app=app;this.currentTab='logic';}
setTab(tab){this.currentTab=tab;this.render();}
render(){const container=document.getElementById('materials-view-container');if(!container)return;const lang=this.app.currentLanguage;const isId=lang==='id';const tabs=[{id:'logic',name_id:'Logika & Figural CPNS',name_en:'Logic & Spatial Reasoning'},{id:'comp_thinking',name_id:'Computational Thinking',name_en:'Computational Thinking'},{id:'ai_math',name_id:'Konsep AI & Matematika',name_en:'AI Concepts & Linear Algebra'},{id:'swift',name_id:'Pemrograman Swift 6',name_en:'Swift 6 Modern Programming'},{id:'oop',name_id:'OOP & Software Architecture',name_en:'OOP & Software Architecture'}];let tabsHtml='<div class="materials-tabs-bar">';for(const t of tabs){const isActive=this.currentTab===t.id;const title=isId?t.name_id:t.name_en;tabsHtml+=`
        <button class="material-tab-btn ${isActive ? 'active' : ''}" data-tab="${t.id}">
          <span>${title}</span>
        </button>
      `;}
tabsHtml+='</div>';const contentHtml=this.getTabContent(this.currentTab,isId);container.innerHTML=`
    ...
    `;document.getElementById('btn-materials-back')?.addEventListener('click',()=>{this.app.navigate('portal');});container.querySelectorAll('.material-tab-btn').forEach(btn=>{btn.addEventListener('click',()=>{const tabId=btn.getAttribute('data-tab');if(tabId)this.setTab(tabId);});});container.querySelectorAll('.btn-jump-practice').forEach(btn=>{btn.addEventListener('click',()=>{this.app.navigate('portal');});});}
getTabContent(tab,isId){switch(tab){case'logic':return this.getLogicContent(isId);case'comp_thinking':return this.getCTContent(isId);case'ai_math':return this.getAIMathContent(isId);case'swift':return this.getSwiftContent(isId);case'oop':return this.getOOPContent(isId);default:return'';}}
getLogicContent(isId){if(isId){return`
      ...
      `;}}
getCTContent(isId){if(isId){return`
        <div class="material-article">
...
print(total) // What is the final value of 'total'?</div>
      ...
      `;}}
getAIMathContent(isId){if(isId){return`
        <div class="material-article">
      ...
      `;}}
getSwiftContent(isId){if(isId){return`
        <div class="material-article">
...
let name = optionalVal ?? "Default"</div>
      ...
      `;}}
getOOPContent(isId){if(isId){return`
        <div class="material-article">
